...
import math
#Warning
print('Some really, really complex equations may not be balanced by this program. However, 99% of equations can be balanced.')
print('Note that this program is not compatible for square brackets.')
...
for ind in range(len(reactants_list)):
    for string in reactants_list[ind]:
        if p==0:
            ...
            #Inside of brackets
            #Checking for elements and quantities
            if string.isdigit() and poly_check==False:
                quantity=quantity+str(string)
            elif string.isalpha() and poly_check==False:
                if string.isupper():
                    if element=='':
                        element=element+string
                    else:
                        if element in poly_elements:
                            o_quantity=poly_quantity[poly_elements[ind].index(element)]
                            if quantity=='':
                                poly_quantity[poly_elements[ind].index(element)]=int(1+o_quantity)
                            else:
                                poly_quantity[poly_elements[ind].index(element)]=int(int(quantity)+o_quantity)
                        else:
                            o_quantity=0
                            poly_elements.append(element)
                            if quantity=='':
                                poly_quantity.append(int(1))
                            else:
                                poly_quantity.append(int(int(quantity)))
                        element=''
                        quantity=''
                        element=element+string
                elif string.islower():
                    element=element+string
            elif string.isdigit() and poly_check==True:
                poly_scaler=poly_scaler+string
            elif string==')':
                poly_check=True
                if element in poly_elements:
                    o_quantity=poly_quantity[poly_elements[ind].index(element)]
                ...
                p=0
                for i in range(len(poly_quantity)):
                    poly_quantity[i]=int(poly_quantity[i]*int(poly_scaler))
                for i in range(len(poly_elements)):
                    if poly_elements[i] not in e_reactants[ind]:
                        e_reactants[ind].append(poly_elements[i])
                        eq_reactants[ind].append(poly_quantity[i])
                    else:
                        o_quantity=eq_reactants[ind][e_reactants[ind].index(poly_elements[i])]
                        eq_reactants[ind][e_reactants[ind].index(poly_elements[i])]=o_quantity+poly_quantity[i]
                p=0
                poly_elements=[]
                poly_quantity=[]
                poly_check=False
    ...
    element=''
    quantity=''
temp_e_list=[[] for i in e_reactants]
temp_q_list=[[] for i in eq_reactants]
for ind in range(len(e_reactants)):
    for i in range(len(e_reactants[ind])): 
        if e_reactants[ind][i]!='':
            temp_e_list[ind].append(e_reactants[ind][i])
            temp_q_list[ind].append(eq_reactants[ind][i])
e_reactants=temp_e_list
eq_reactants=temp_q_list
...
for ind in range(len(products_list)):
        ...
        if p==0:
                ...
                if string.isupper():
                    if element=='':
                        ...
                        if element in e_products[ind]:
                            ...
                            if quantity=='':
                                eq_products[ind].append(int(1))
                            else:
                                eq_products[ind].append(int(int(quantity)))
                        element=''
                        quantity=''
                        element=element+string
                elif string.islower():
            ...
            if string.isdigit() and poly_check==False:
                ...
                for i in range(len(poly_quantity)):
                    poly_quantity[i]=int(poly_quantity[i]*int(poly_scaler))
                for i in range(len(poly_elements)):
                    if poly_elements[i] not in e_products[ind]:
                        e_products[ind].append(poly_elements[i])
                        eq_products[ind].append(poly_quantity[i])
                    else:
                        o_quantity=eq_products[ind][e_products[ind].index(poly_elements[i])]
                        eq_products[ind][e_products[ind].index(poly_elements[i])]=o_quantity+poly_quantity[i]
                p=0
                poly_elements=[]
                poly_quantity=[]
                poly_check=False
    ...
    element=''
    quantity=''
temp_e_list=[[] for i in e_products]
temp_q_list=[[] for i in eq_products]
for ind in range(len(e_products)):
    for i in range(len(e_products[ind])): 
        if e_products[ind][i]!='':
            temp_e_list[ind].append(e_products[ind][i])
            temp_q_list[ind].append(eq_products[ind][i])
e_products=temp_e_list
eq_products=temp_q_list
