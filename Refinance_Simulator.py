...
for u in range((len(time_list))):
    ...
    if u == (len(time_list) - 1):
        ...
        principle_limit = round(float(money * (((1+minterest_rate)**time) -1)) / float(minterest_rate * ((1+minterest_rate)**time)))
    ...
    for x in range(time):
        ...
        if loan_number == 1:
            ...
            l_total_interest_list.append(total_interest)
        else:
            ltotal_interest = total_interest - total_interest_1
            l_total_interest_list.append(ltotal_interest)
    index = index + 1
print(loan_number_list, loan_time_list, a_interest_rate_list, principle_limit_list, current_principle_list, mtime_list, interest_list, money_list, total_interest_list, money_paid_list, l_total_interest_list)
