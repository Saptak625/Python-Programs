...
starting_letter_choice=str(input("Please enter the starting letter used in encryption:"))
...
print("Was a keyword or a keylist used in encrytion? If it is a keyword, then type 'KEA'. if it is a keylist, then type in RNG.")
keyword_gen=str(input("KEA or RNG?:"))
keyword_gen=keyword_gen.upper()
if keyword_gen == "KEA":
    #Asks keyword and converts it into all lowercase
    mystr=str(input("Enter the Keyword used in Encryption:"))
    mystr=mystr.lower()
    check_str=mystr [::-1]
    if check_str == 'scivesakulybba':
        oldstr = mystr
        mystr = "gfdzhdzgfhdzhzhfgd"
